...
#endif
#if Application == 28
#include <include/RazorMultiB.hh>
#endif
#if Application == 29
#include <include/MonoJet.hh>
#endif
#if Application == 30
...
#if Application == 8  
  if(isData) {
    Razor vecbos(theChain, string(json), true, true);
    vecbos.Loop(string(outFileName), start, stop);  
  } else {
...
#if Application == 29
  MonoJet vecbos(theChain, "none", isData, isData);
  vecbos.SetWeight(double(weight));
  vecbos.Loop(string(outFileName), start, stop);
#endif

#if Application == 30
  RazorDiMuB vecbos(theChain, "none", isData, isData);
  vecbos.SetWeight(double(weight));
  vecbos.Loop(string(outFileName), start, stop);
#endif
