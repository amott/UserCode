...
vector<Jet> Vecbos::GetUncorrPFJets() {
  ...
  for(int j=0; j<nAK5PFJet; j++) {
    ...
    TLorentzVector p4;
    p4.SetPtEtaPhiE(uncorrEt,etaAK5PFJet[j],phiAK5PFJet[j],uncorrEnergyAK5PFJet[j]);
    float totenergy = (chargedHadronEnergyAK5PFJet[j]+neutralHadronEnergyAK5PFJet[j]+
                       chargedEmEnergyAK5PFJet[j]+neutralEmEnergyAK5PFJet[j]);
    float efrac = (chargedEmEnergyAK5PFJet[j]+neutralEmEnergyAK5PFJet[j]) / totenergy;
    float hfrac = (chargedHadronEnergyAK5PFJet[j]+neutralHadronEnergyAK5PFJet[j]) / totenergy;
    ...
    float nConstituents = chargedHadronMultiplicityAK5PFJet[j] + neutralHadronMultiplicityAK5PFJet[j] +
    ...
    float chargedMultiplicity = chargedHadronMultiplicityAK5PFJet[j] + electronMultiplicityAK5PFJet[j] + muonMultiplicityAK5PFJet[j];
    float chargedEmFraction = chargedEmEnergyAK5PFJet[j]/energyAK5PFJet[j];
    sisconej.setPFJetID(neutralHadFrac,neutralEmFraction,nConstituents,
...
vector<Jet> Vecbos::GetCorrPFJets(int scaleEnergy) {
  ...
  for(int j=0; j<nAK5PFJet; j++) {
    TLorentzVector p4;
    if(scaleEnergy==0) p4.SetPxPyPzE(pxAK5PFJet[j],pyAK5PFJet[j],pzAK5PFJet[j],energyAK5PFJet[j]);
    else {
      float mass = sqrt(pow(energyAK5PFJet[j],2)-pow(pxAK5PFJet[j],2)-pow(pyAK5PFJet[j],2)-pow(pzAK5PFJet[j],2));
      float ptUnscaled = sqrt(pow(pxAK5PFJet[j],2)+pow(pyAK5PFJet[j],2));

      // estimate the uncertainty
      jecUnc_PF->setJetEta(etaAK5PFJet[j]);
      jecUnc_PF->setJetPt(ptUnscaled);

      // apply the uncertainty
      float pt = ptUnscaled + scaleEnergy*jecUnc_PF->getUncertainty(true)*ptUnscaled;
      float p = pt/fabs(sin(thetaAK5PFJet[j]));
      float energy = sqrt(p*p+mass*mass);

      p4.SetPtEtaPhiE(pt,etaAK5PFJet[j],phiAK5PFJet[j],energy);
    }
    float totenergy = (chargedHadronEnergyAK5PFJet[j]+neutralHadronEnergyAK5PFJet[j]+
                       chargedEmEnergyAK5PFJet[j]+neutralEmEnergyAK5PFJet[j]);
    float efrac = (chargedEmEnergyAK5PFJet[j]+neutralEmEnergyAK5PFJet[j]) / totenergy;
...
vector<BTagJet> Vecbos::GetBTagCorrPFJets()
{
  ...
  for(int j=0; j<nAK5PFJet; j++) {
    BTagJet btag;
    btag.combinedSecondaryVertexBJetTags = combinedSecondaryVertexBJetTagsAK5PFJet[j];
    btag.combinedSecondaryVertexMVABJetTags = combinedSecondaryVertexMVABJetTagsAK5PFJet[j];
    btag.jetBProbabilityBJetTags = jetBProbabilityBJetTagsAK5PFJet[j];
    btag.jetProbabilityBJetTags = jetProbabilityBJetTagsAK5PFJet[j];
    btag.simpleSecondaryVertexBJetTags = simpleSecondaryVertexHighEffBJetTagsAK5PFJet[j];
    btag.softMuonBJetTags = softMuonBJetTagsAK5PFJet[j];
    btag.trackCountingHighPurBJetTags = trackCountingHighPurBJetTagsAK5PFJet[j];
    btag.trackCountingHighEffBJetTags = trackCountingHighEffBJetTagsAK5PFJet[j];
    btags.push_back(btag);
  }
...
{
   if(index < 0 || index >= nAK5PFJet)                     return false;
   if(trackCountingHighEffBJetTagsAK5PFJet[index] < 1.7)   return false;
   return true;
}
...
{
   if(index < 0 || index >= nAK5PFJet)                     return false;
   if(trackCountingHighEffBJetTagsAK5PFJet[index] < 3.3)   return false;
   return true;
}
...
{
   if(index < 0 || index >= nAK5PFJet)                      return false;
   if(trackCountingHighEffBJetTagsAK5PFJet[index] < 10.2)   return false;
   return true;
}
...
{
   if(index < 0 || index >= nAK5PFJet)                      return false;
   if(trackCountingHighPurBJetTagsAK5PFJet[index] < 1.19)   return false;
   return true;
}
...
{
   if(index < 0 || index >= nAK5PFJet)                      return false;
   if(trackCountingHighPurBJetTagsAK5PFJet[index] < 1.93)   return false;
   return true;
}
...
{
   if(index < 0 || index >= nAK5PFJet)                      return false;
   if(trackCountingHighPurBJetTagsAK5PFJet[index] < 3.41)   return false;
   return true;
}
...
{
   if(index < 0 || index >= nAK5PFJet)                              return false;
   if(simpleSecondaryVertexHighEffBJetTagsAK5PFJet[index] < 1.74)   return false;
   return true;
}
...
{
   if(index < 0 || index >= nAK5PFJet)                              return false;
   if(simpleSecondaryVertexHighEffBJetTagsAK5PFJet[index] < 3.05)   return false;
   return true;
}
...
{
   if(index < 0 || index >= nAK5PFJet)                              return false;
   if(simpleSecondaryVertexHighPurBJetTagsAK5PFJet[index] < 2.00)   return false;
   return true;
}
*/
// BTAG 2012 Working Points
bool Vecbos::pfJetPassCSVL(double btagOutput)
{  if(btagOutput < 0.244)   return false;
   return true;
}

bool Vecbos::pfJetPassCSVM(double btagOutput)
{  if(btagOutput < 0.679)   return false;
   return true;
}


bool Vecbos::pfJetPassCSVT(double btagOutput)
{  if(btagOutput < 0.898)   return false;
   return true;
}
...
void Vecbos::isMuonID2012(int muonIndex, bool *muonIdOutput) {
  ...
  *muonIdOutput = true;
  ...
  if(anaUtils.muonIdVal(muonIdMuon[muonIndex],bits::AllGlobalMuons)) {
  ...
  if( (anaUtils.muonIdVal(muonIdMuon[muonIndex],bits::AllTrackerMuons) &&
       anaUtils.muonIdVal(muonIdMuon[muonIndex],bits::TMLastStationTight)) ) flagTrackerMu  = true;
...
bool Vecbos::isTrigElectron(int iEle){
  ...
  if( fabs(p3Ele.Eta()) > 2.5 ) PASS = false;
  if( p3Ele.Pt() < 10.  ) PASS = false;
  ...
  // sigmaIetaIeta
  if ( isEleEB && thisSigmaIeIe>0.01) PASS = false;
  if (!isEleEB && thisSigmaIeIe>0.03) PASS = false;
  ...
  if (ecalIsol>0.2) PASS = false;
  if (hcalIsol>0.2) PASS = false;
  if (trackerIsol>0.2) PASS = false;

  // H/E
  if ( isEleEB && hOverEEle[theEle]>0.12) PASS = false;
  if (!isEleEB && hOverEEle[theEle]>0.10) PASS = false;

  // deltaEta
  if ( isEleEB && (fabs(deltaEtaAtVtxEle[theEle])>0.007) ) PASS = false;
  if (!isEleEB && (fabs(deltaEtaAtVtxEle[theEle])>0.009) ) PASS = false;

  // deltaPhi
  if ( isEleEB && (fabs(deltaPhiAtVtxEle[theEle])>0.15) ) PASS = false;
  if (!isEleEB && (fabs(deltaPhiAtVtxEle[theEle])>0.10) ) PASS = false;
...
//WP 80 https://twiki.cern.ch/twiki/bin/view/Main/HVVElectronId2012#Triggering_electrons_MVA
bool Vecbos::isTightElectron(int iEle){

  bool PRE = isTrigElectron(iEle);
  if(!PRE) return false;

  float bdt = mvaidtrigEle[iEle];
  ...
  float iso = pfCandChargedIso04Ele[iEle];
  //  iso += max<float>(0.,pfCandNeutralIso04Ele[iEle]-eff_area_nh*rhoJetsFastJet + pfCandPhotonIso04Ele[iEle]-eff_area_ga*rhoJetsFastJet);
  iso += max<float>(0.,pfCandNeutralIso04Ele[iEle]+pfCandPhotonIso04Ele[iEle]-eff_area_ganh*rhoJetsFastJet);
  ...
  bool ELE_ID_EGAMMA_2012 = (
                             ( pt <= 20 && abseta >= 0.000 && abseta < 0.800 && bdt > 0.013 ) ||
                             ( pt <= 20 && abseta >= 0.800 && abseta < 1.479 && bdt > 0.308 ) ||
                             ( pt <= 20 && abseta >= 1.479 && abseta < 2.500 && bdt > 0.397 ) ||
                             ( pt >  20 && abseta >= 0.000 && abseta < 0.800 && bdt > 0.913 ) ||
                             ( pt >  20 && abseta >= 0.800 && abseta < 1.479 && bdt > 0.964 ) ||
                             ( pt >  20 && abseta >= 1.479 && abseta < 2.500 && bdt > 0.899 )
                             );
  bool ELE_ISO_EGAMMA_2012 = (
                             ( pt <= 20 && abseta >= 0.000 && abseta < 0.800 && iso < 0.167 ) ||
                             ( pt <= 20 && abseta >= 0.800 && abseta < 1.479 && iso < 0.213 ) ||
                             ( pt <= 20 && abseta >= 1.479 && abseta < 2.500 && iso < 0.233 ) ||
                             ( pt >  20 && abseta >= 0.000 && abseta < 0.800 && iso < 0.105 ) ||
                             ( pt >  20 && abseta >= 0.800 && abseta < 1.479 && iso < 0.178 ) ||
                             ( pt >  20 && abseta >= 1.479 && abseta < 2.500 && iso < 0.150 )
                             );

  int gsf = gsfTrackIndexEle[iEle];
  bool ELE_CONV_2012 = (!hasMatchedConversionEle[iEle] && expInnerLayersGsfTrack[gsf]==0);

  if(ELE_ID_EGAMMA_2012 && ELE_ISO_EGAMMA_2012 && ELE_CONV_2012){
    return true;
  } else {
    return false;
  }
...
//WP 95 https://twiki.cern.ch/twiki/bin/view/Main/HVVElectronId2012#Triggering_electrons_MVA
bool Vecbos::isLooseElectron(int iEle){
  ...
  bool ELE_ISO_EGAMMA_2012 = (
                             ( pt <= 20 && abseta >= 0.000 && abseta < 0.800 && iso < 0.408 ) ||
                             ( pt <= 20 && abseta >= 0.800 && abseta < 1.479 && iso < 2.375 ) ||
                             ( pt <= 20 && abseta >= 1.479 && abseta < 2.500 && iso < 0.463 ) ||
                             ( pt >  20 && abseta >= 0.000 && abseta < 0.800 && iso < 0.253 ) ||
                             ( pt >  20 && abseta >= 0.800 && abseta < 1.479 && iso < 0.225 ) ||
                             ( pt >  20 && abseta >= 1.479 && abseta < 2.500 && iso < 0.308 )
                             );
...
bool Vecbos::isLooseTau(int iTau){
  if(abs(thehpsTauDiscrByTightElectronRejectionPFTau[iTau]-1.) < 0.0001 && 
     ...
     abs(thehpsTauDiscrByLooseCombinedIsolationDBSumPtCorrPFTau[iTau]-1.)<0.0001) {
    if(fabs(etaPFTau[iTau]) < 2.3){
      return true;
    }
  }
...
bool Vecbos::isMediumTau(int iTau){
  if(abs(thehpsTauDiscrByTightElectronRejectionPFTau[iTau]-1.) < 0.0001 && 
     abs(thehpsTauDiscrByTightMuonRejectionPFTau[iTau]-1.) < 0.0001 &&
     abs(thehpsTauDiscrByMediumCombinedIsolationDBSumPtCorrPFTau[iTau]-1.)<0.0001) {
    if(fabs(etaPFTau[iTau]) < 2.3){
      return true;
    }
  }
...
bool Vecbos::isTightTau(int iTau){
  if(abs(thehpsTauDiscrByTightElectronRejectionPFTau[iTau]-1.) < 0.0001 && 
     abs(thehpsTauDiscrByTightMuonRejectionPFTau[iTau]-1.) < 0.0001 &&
     abs(thehpsTauDiscrByTightCombinedIsolationDBSumPtCorrPFTau[iTau]-1.)<0.0001) {
    if(fabs(etaPFTau[iTau]) < 2.3){
      return true;
    }
  }
