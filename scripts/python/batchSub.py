    ...
    for i in range(NLists):
        ...
        if submitMode: # do the actual submission
            if __SITE__ == "CALTECH":
                ...
                os.system(bsub)
            if __SITE__ =="CERN":
                bsub = "bsub -q %s -o %s/log -e %s/log source %s" % (queue, DirName, DirName, scriptFile)
                print bsub
                os.system(bsub)

            
if __name__=="__main__":
    if not __SITE__ in __VALID_SITES__:
        print "INVALID __SITE__ VARIABLE IN SCRIPT"
        print "PLEASE SET __SITE__ CORRECTLY AND TRY AGAIN"
        sys.exit(0)
    main()
