...
std::vector<std::pair<int,float> > HggVertexing::evalPerVtxMVA(VecbosPho* pho1, VecbosPho* pho2){
  ...
  for(int iVtx=0; iVtx<base->nPV; iVtx++){
    ...
    if(debugVertexing) std::cout<< "Pho1 Pt: " << pho1_p4.Pt() << "   Pho2 Pt: " << pho2_p4.Pt() << std::endl;
...
float HggVertexing::convCorrectedDz(VecbosConversion* c,TVector3 basePos){
  ...
  return (c->getVertex().Z()-basePos.Z()) - ((c->getVertex().X()-basePos.X())*c->pRefittedPair.Px() + (c->getVertex().Y()-basePos.Y())*c->pRefittedPair.Py())/c->pRefittedPair.Pt()*c->pRefittedPair.Pz()/c->pRefittedPair.Pt();

}

float HggVertexing::Z0EcalVtxCiC(VecbosConversion* c, TVector3 basePos,TVector3 caloPos){
  float dx1 = caloPos.X() - c->getVertex().X();
  float dy1 = caloPos.Y() - c->getVertex().Y();
  float dz1 = caloPos.Z() - c->getVertex().Z();
  float r1 = sqrt(dx1*dx1+dy1*dy1);
  float tantheta = r1/dz1;

  float dx2 = c->getVertex().X()-basePos.X();
  float dy2 = c->getVertex().Y()-basePos.Y();
